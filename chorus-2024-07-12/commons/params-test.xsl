...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="vendorName" select="//article/front/journal-meta/journal-id"/>
    <xsl:param name="archiveFile" select="replace(saxon:system-id(), '(.*/)(.*)(\.xml)','$2')"/>
    <xsl:param name="originalFilename" select="saxon:system-id()"/>
    <xsl:param name="workingDir" select="replace(saxon:system-id(),'(.*/)(.*)(\.xml)','$1')"/>
</xsl:stylesheet>

...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.loc.gov/mods/v3" xmlns:date="http://exslt.org/dates-and-times" xmlns:f="http://functions" xmlns:mods="http://www.loc.gov/mods/v3"  xmlns:regexp="http://exslt.org/regular-expressions" xmlns:saxon="http://saxon.sf.net/" xmlns:str="http://exslt.org/strings" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" 
    extension-element-prefixes="date str regexp" exclude-result-prefixes="f saxon mods xd xlink xs xsi">
    <xsl:import href="commons/1.0/str/str.xsl"/>
    ...
    <xsl:template match="licenseUrl">
        <xsl:if test="not(. = '') and contains(., 'creativecommmons') or contains(., 'openaccess') or contains(., 'vor') or contains(., 'chorus')">
    ...
    <xsl:template match="funders">
        <funding-group>
            <xsl:for-each select="../funders/item[generate-id()=generate-id(key('name',text())[1])]">
...
-->
    <xd:doc>      <!-- 1.23-->
        <xd:desc>tokenize-dates</xd:desc>
    ...
    <xsl:template name="tokenize-dates">
        ...
        <xsl:variable name="token1">
            ...
            <xsl:choose>
                <xsl:when test="string-length($preEvalToken1) = 1">
                    <xsl:value-of select="concat('0',$preEvalToken1)"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="$preEvalToken1"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>            

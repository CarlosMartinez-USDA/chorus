            ...
            <accessCondition type="use and reproduction" displayLabel="CHORUS License Information">
                ...
                <program xmlns="https://data.crossref.org/schemas/AccessIndicators.xsd">
                    <xsl:element name="license_ref">
                        <xsl:attribute name="applies_to" select="license_type"/>
                           <!-- <xsl:variable name="license_type" select="/all/license_type[@type = 'str']"/>
                            <xsl:value-of select="$license_type"/>
                        </xsl:attribute>-->
                        <xsl:variable name="access_date" as="xs:date" select="xs:date(substring-before(publicly_accessible_on_publisher_site, 'T'))"/> 
                        <xsl:attribute name="start_date" select="format-date($access_date, '[Y,4]-[M,2]-[D,2]')"/>>
                           <!-- <xsl:variable name="access_date" select="format-date($access_date, '[Y,4]-[M,2]-[D,2]')"/>
                    ...
                    <xsl:element name="license_ref">
                        ...
                        <xsl:variable name="reuse" as="xs:date" select="xs:date(substring-before(reuse_license_start_date, 'T'))"/> 
                        <xsl:attribute name="start_date" select="format-date($reuse, '[Y,4]-[M,2]-[D,2]')"/>

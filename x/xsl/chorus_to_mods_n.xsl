...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    exclude-result-prefixes="f saxon mods xd xlink xs xsi">
    ...
    <xd:doc scope="stylesheet" id="chorus">
        <xd:desc>
            <xd:p>
                <xd:b>CHORUS to MODS XML Transformation:</xd:b>
            </xd:p>
            ...
            <xd:p>
                <xd:b>Filename: </xd:b>
                <xd:i>chorus_to_mods.xsl</xd:i>
            </xd:p>
            <xd:p>
                <xd:b>Change log:</xd:b>
            </xd:p>
            <xd:ul>
                <xd:li>
                    <xd:p>Added conditional statement to remvoe "From the..." in affiliations -
                        20240509 - cm3</xd:p>
                </xd:li>
                <xd:li>
                    <xd:p>Funders template adds &lt;institution_id @type='doi'&gt;. - 20240430 -
                        cm3</xd:p>
                </xd:li>
                <xd:li>
                    <xd:p>Added a-file output. - 20240430 - cm3</xd:p>
                </xd:li>
                <xd:li>
                    <xd:p>Authors' name template tokenized for first ane last, substring-after for
                        middleParts. - 20240418 - cm3</xd:p>
                </xd:li>
                <xd:li>
                    <xd:p>Collection processing added (currently commented out). - 20240418 -
                        cm3</xd:p>
                </xd:li>
                <xd:li>
                    <xd:p>Upgraded XSLT version to 2.0. - 20240418 - cm3</xd:p>
                </xd:li>
                <xd:li>
                    <xd:p>Change log added. - 20240418 - cm3</xd:p>
                </xd:li>
            </xd:ul>
        </xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>
            <xd:p>
                <xd:b>Root template selects individual CHORUS XML.</xd:b>
            </xd:p>
            <xd:p>
                <xd:b>Uncomment collection processing lines when necessary.</xd:b>
            </xd:p>
        </xd:desc>
    </xd:doc>
    <xsl:template match="/">
    ...
    <xd:doc id="author" scope="component">
        <xd:desc>
            <xd:p><xd:b>(1)</xd:b> Tokenize &lt;author&gt;. Set non-breaking-whitespace as
                tokenizationdelimiter.</xd:p>
            <xd:p><xd:b>(2)</xd:b> Element is parsed into &lt;namePart&gt; and &lt;displayForm&gt;
                elements. </xd:p>
            <xd:p><xd:b>(3)</xd:b> The delimiter splits each name component into the following
                nameParts:</xd:p>
            <xd:ul>
                <xd:li>
                    <xd:p><xd:b>a.</xd:b> $name-tokens[1] is the $familyName</xd:p>
                </xd:li>
                <xd:li>
                    <xd:p><xd:b>b.</xd:b> $name-tokens[2] is the first name, or part of the
                        $givenName</xd:p>
                </xd:li>
                <xd:li>
                    <xd:p><xd:b>c.</xd:b> $givenName = combined $names-token[2] and the rest of the
                        string after it.)</xd:p>
                </xd:li>
            </xd:ul>
        </xd:desc>
    </xd:doc>
    <xsl:template match="authors">
        <xsl:for-each select="item">
            ...
            <xsl:variable name="firstMiddle"
                select="concat($name-tokens[2], substring-after(author, $name-tokens[2]))"/>
            <name type="personal">
                ...
                <xsl:if test="matches($firstMiddle, '[A-z]+\.?')">
                    <!-- add punctuation -->
                    <xsl:variable name="givenName">
                        <xsl:sequence select="
                                if (matches($firstMiddle, '^.*\s[A-Z]$') and not(ends-with($firstMiddle, '.')))
                                then
                                    concat($firstMiddle, '.')
                                else
                                    $firstMiddle"/>
                    ...
                    <displayForm>
                        <xsl:value-of
                            select="normalize-space(concat($familyName, ',&#xa0;', $givenName))"/>
                    </displayForm>
    ...
    <xd:doc>
        <xd:desc>
            <xd:p>orcid</xd:p>
            <xd:p>updated 20240418 added for-each and conditional XPath</xd:p>
        </xd:desc>
        <xd:param name="last"/>
        <xd:param name="first"/>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>published_print</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>published_online</xd:desc>
    </xd:doc>
                    ...
                    <funding-source>
                        <institution-wrap>
                            ...
                            <institution_id type="doi">
                                <xsl:value-of
                                    select="concat('https://doi.org/', subsequence(../$axis/item[$pos], 1, 1))"
                                />
                            </institution_id>
    ...
    <xsl:template name="recordInfo">
        <recordInfo>
            ...
            <recordOrigin>
                <xsl:text>XML source generated via Python using CHORUS API JSON data; converted to MODS with chorus_to_mods.xsl</xsl:text>
            </recordOrigin>
        </recordInfo>
    </xsl:template>

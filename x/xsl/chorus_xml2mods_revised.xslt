...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xd:doc scope="stylesheet" id="chorus">
        <xd:desc>
            ...
            <xd:ul>
                <xd:p>
                    <xd:b>CHORUS XML Transformations</xd:b>
                </xd:p>
                <xd:li>
                    <xd:p><xd:i>chorus_xml2mods.xsl:</xd:i> XSLT to transforms CHORUS XML to
                        MODS.</xd:p>
                </xd:li>
                <xd:li>
                    <xd:p><xd:i>NAL_mods2marc21slim.xsl</xd:i> XSLT to MODS to MARC, from CHORUS
                        source XML</xd:p>
                </xd:li>
            </xd:ul>
        </xd:desc>
    ...
    <xd:doc>
        <xd:desc>
            ...
            <xd:p>
                <xd:b>Commented out section would allow handling of multiple CHORUS XML
                    documents.</xd:b>
            </xd:p>
        </xd:desc>
    </xd:doc>
    <xsl:template match="/">
        <!-- uncomment for collection processsing -->
        <xsl:choose>
    ...
    <xd:doc>
        <xd:desc>name item-info</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>match authors</xd:desc>
    </xd:doc>
    <xsl:template match="authors">
        <xsl:for-each select="item">
            <xsl:variable name="name-tokens" select="tokenize(author, '&#xa0;')"/>
            <xsl:variable name="given"
                select="normalize-space(substring-after(author, $name-tokens[1]))"/>
    ...
    <!--    NEW TEMPLATE:
            (1)This new template parses &lt;author&gt; into &lt;namePart&gt; elements.
            (2)The analyze-string processing instruction allows integration of both 
    ...
    <xd:doc>
        <xd:desc>
            <xd:p><xd:b>Author names template: </xd:b>introduces &lt;xsl:analyze-strin&gt;
                processing-instruction.</xd:p>
            <xd:p><xd:b>&lt;xsl:analyze-string&gt;</xd:b> sub-elements allows for two methods to
                parse names.</xd:p>
            <xd:ul>
                <xd:li>
                    <xd:p><xd:i>&lt;xsl:matching-substring&gt; - </xd:i>the regex attribute parses
                        the authors name into separate parts.</xd:p>
                </xd:li>
                <xd:li>
                    <xd:p><xd:i>&lt;xsl:non=matching-substring&gt; - </xd:i>re-enables and updates
                        the tokenize function to capture separate name parts. </xd:p>
                </xd:li>
            </xd:ul>
        </xd:desc>
    </xd:doc>
    <xsl:template match="author">
        <!-- regex attribute parses name into two variables {$lastName},{{$firstName}{ $i}?} -->
        <xsl:analyze-string select="." regex="^(\S+)\s(\S+)(\s[A-Z])?\.?$">
            <xsl:matching-substring>
                ...
                <xsl:variable name="firstName">
                    <xsl:sequence select="
                            if (regex-group(3) != '')
                            then
                                concat(regex-group(2), regex-group(3), '.')
                            else
                                regex-group(2)"/>
                </xsl:variable>
            ...
            <xsl:non-matching-substring>
                ...
                <xsl:variable name="firstName" select="
                        if ($name-tokens[3] != '')
                        then
                            concat($name-tokens[2], '&#xa0;', translate($name-tokens[3], '.', ''), '.')
                        else
                            $name-tokens[2]"/>
                <namePart type="family">
                    <xsl:value-of select="$lastName"/>
                ...
                <xsl:choose>
                    <xsl:when test="$name-tokens[3] != ''">
                        ...
                        <displayForm>
                            <xsl:value-of
                                select="normalize-space(concat($lastName, '&#xa0;', $firstName))"/>
                        </displayForm>
                    </xsl:when>
                    <xsl:otherwise>
                        <namePart type="given">
    ...
    <xd:doc>
        <xd:desc>match affiliation</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>match licenseUrl</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>name originInfo</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>match published_print</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>match published_online</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>name extension</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>match agency_id</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>match agency_name</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>match breakdown_for</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>name recordInfo</xd:desc>
    </xd:doc>
    <xsl:template name="recordInfo">
        <recordInfo>
            ...
            <recordOrigin>XML source generated via Python using CHORUS API JSON data; converted to
                MODS with chorus_to_mods.xsl</recordOrigin>
        </recordInfo>
    </xsl:template>
